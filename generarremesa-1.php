...
function CreaRemesa($idRemesa, $orde, $v, $recibos) {
    ...
    $arrayRemesa = array(
        'header' => array(
            ...
            'cif' => "ES" . digitoControlPresentador($orde['Cif']) .$orde['Sufijo'] . $orde['Cif'],
            'iban' => $orde['Iban'],
            'bic' => $orde['Bic'],
        ),
        'recibos' => $recibos,
    ...
    $fichero = $sepa->makeDocument("remesas/".$idRemesa.".xml", $arrayRemesa);
...
switch ($v['accion']) {
    case 'Si':
        ...
        if ($error == "") {
            ...
            $res = mysql_query($sql);
            while ($orde = mysql_fetch_assoc($res)) {
                ...
                $idRemesa = $orde['Cif'] . date('Y-m-d') . "T" . date('His');
                ...
                $recibos = array();
                while ($recibo = mysql_fetch_assoc($res1)) {
                    $recibos[] = array(
                    ...
                    $query = "update " . $_SESSION['DBDAT'] . $orde['IDEmpresa'] . ".recibos set IDRemesa='" . $idRemesa . "' where IDRecibo='" . $recibo['IDRecibo'] . "' limit 1;";
                    //echo $query,"<br/>";
                    $actu = mysql_query($query);
                    if (!$actu)
                        Mensaje("NO SE HA ACTUALIZADO EL RECIBO " . $recibos['IDRecibo'] . " de la empresa " . $orde['IDEmpresa']);
                }

                if (count($recibos)) {
                    $resultado[] = CreaRemesa($idRemesa,$orde, $v, $recibos);
                }
            }
            //FIN BUCLE ORDENANTE
