<?
    ...
    switch ($accion){
        case "Borrar":
            @unlink('remesas/'.$f);
            //QUITAR LA DE REMESADO EN LOS RECIBOS. TENGO QUE RECORRER TODAS LAS EMPRESAS.
            $idRemesa = basename($f,'.xml');
            $sql="select IDEmpresa from $EMP.empresas";
            $res=mysql_query($sql);
            while ($emp=mysql_fetch_array($res)){
                $sql="update $DAT".$emp[0].".recibos set IDRemesa='' where IDRemesa='$idRemesa';";
                //echo $sql,"<br/>";
                mysql_query($sql);
            }
        break;
